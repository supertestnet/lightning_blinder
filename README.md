...
LN Proxy (https://github.com/lnproxy) is similar software that *also* hides the "real" recipient from people like LSPs. But not a lot of people run LN Proxy and I think that's largely because it takes special software -- you have to install LND (because other LN implementations don't natively support hodl invoices), run LN Proxy on top of it, and configure a web server to expose it so folks can use it. I want to eliminate those requirements for LSP Blinder.
...
All the "important" logic will be handled on the client side, by the sender and the recipient, and I can build a prototype wallet to demonstrate that the protocol works. Then other wallets can implement support for this protocol if there's demand, and hopefully privacy on the lightning network will improve thereby.
