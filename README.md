...
If coordinators simply make their feerates public (probably via nostr), users can see a list of them and pick one or more coordinators based on their feerates. All the "important" logic can thus be handled on the client side, by the sender and the recipient. The prototype in the html file demonstrates that the protocol works, and I hope other wallets implement support for this protocol -- or perhaps someone will make a "real" version of the prototype that runs on a phone independently of any wallet backend. Hopefully, privacy on the lightning network will improve as a result.
